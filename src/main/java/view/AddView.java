...
public class AddView  implements ActionList{
	...
	public Task getTask() {
		...
		Task task;
		if (getActivefield()){
			task = new Task(getTitleField(),new Date(getStartTimeField()), new Date(getEndTimeField()), getRepeatIntervalField());
			task.setActive(getActivefield());
		} else {
			task = new Task(getTitleField(),new Date(getStartTimeField()));
		}
		return task;
	}
